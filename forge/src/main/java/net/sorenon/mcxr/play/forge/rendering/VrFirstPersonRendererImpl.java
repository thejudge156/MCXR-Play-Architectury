package net.sorenon.mcxr.play.forge.rendering;
...
public class VrFirstPersonRendererImpl implements VrFirstPersonRenderer {

    private static final MCXRGameRenderer XR_RENDERER = MCXR_GAME_RENDERER;
    ...
    public void renderShadow(RenderLevelLastEvent context, Entity camEntity) {
        PoseStack matrices = context.getPoseStack();
        Vec3 camPos = Minecraft.getInstance().gameRenderer.getMainCamera().getPosition();
        matrices.pushPose();
        double x = Mth.lerp(context.getPartialTick(), camEntity.xOld, camEntity.getX());
        double y = Mth.lerp(context.getPartialTick(), camEntity.yOld, camEntity.getY());
        double z = Mth.lerp(context.getPartialTick(), camEntity.zOld, camEntity.getZ());
        matrices.translate(x - camPos.x, y - camPos.y, z - camPos.z);
        PoseStack.Pose entry = matrices.last();

        RenderType SHADOW_LAYER = RenderType.entityShadow(new ResourceLocation("textures/misc/shadow.png"));
        VertexConsumer vertexConsumer = context.getLevelRenderer().renderBuffers.bufferSource().getBuffer(SHADOW_LAYER);
    ...
    public VrFirstPersonRendererImpl(MCXRGuiManager MCXRGuiManager) {
    ...
    @Override
    public void renderLast(Object ctx) {
        RenderLevelLastEvent context = (RenderLevelLastEvent) ctx;
        Camera camera = Minecraft.getInstance().gameRenderer.getMainCamera();
        Entity camEntity = camera.getEntity();
        MultiBufferSource.BufferSource consumers = context.getLevelRenderer().renderBuffers.bufferSource();
        PoseStack matrices = context.getPoseStack();
        ClientLevel world = context.getLevelRenderer().level;
        ...
        if (camEntity != null) {
            ...
            if (!FGM.isScreenOpen() && !MCXRCore.getCoreConfig().controllerRaytracing()) {
                ...
                double x = Mth.lerp(context.getPartialTick(), camEntity.xOld, camEntity.getX());
                double y = Mth.lerp(context.getPartialTick(), camEntity.yOld, camEntity.getY()) + camEntity.getEyeHeight();
                double z = Mth.lerp(context.getPartialTick(), camEntity.zOld, camEntity.getZ());
                matrices.translate(x - camPos.x, y - camPos.y, z - camPos.z);
        ...
        if (camEntity instanceof LocalPlayer player && FGM.isScreenOpen()) {
            render(player, VrFirstPersonRenderer.getLight(camera, world), matrices, consumers, context.getPartialTick());
        }
        ...
        for (int handIndex = 0; handIndex < 2; handIndex++) {
            ...
            if (handIndex != MCXRPlayClient.getMainHand() && XrInput.vanillaGameplayActionSet.teleport.currentState) {
                ...
                if (gripPos.y > hitPos1.y) {
                    ...
                    for (int i = 0; i <= 16; ++i) {
                        VrFirstPersonRenderer.stringVertex((float) hitPos1.x - (float) gripPos.x, (float) hitPos1.y - (float) gripPos.y, (float) hitPos1.z - (float) gripPos.z, consumers.getBuffer(LINE_CUSTOM_ALWAYS.apply(5.)), matrices.last(), i / 16f, (i + 1) / 16f, blocked);
                    }
                } else {
                    matrices.translate((float) hitPos1.x, (float) hitPos1.y, (float) hitPos1.z);
                    for (int i = 0; i <= 16; ++i) {
                        VrFirstPersonRenderer.stringVertex((float) gripPos.x - (float) hitPos1.x, (float) gripPos.y - (float) hitPos1.y, (float) gripPos.z - (float) hitPos1.z, consumers.getBuffer(LINE_CUSTOM_ALWAYS.apply(5.)), matrices.last(), i / 16f, (i + 1) / 16f, blocked);
                    }
                }
            ...
            if (debug) {
                FartUtil.renderCrosshair(consumers, context.getPoseStack(), 0.05f, false);
            }
            ...
            if (debug) {
                ...
                FartUtil.renderCrosshair(consumers, context.getPoseStack(), 0.1f, false);
            }
        ...
        if (!FGM.isScreenOpen() && XrInput.handsActionSet.grip.isActive[0]) {
            ...
            transformToHand(matrices, 0, context.getPartialTick());
    ...
    @Override
    public void renderGuiQuad(PoseStack.Pose transform, MultiBufferSource consumers) {
...
//        consumer.vertex(modelMatrix, -x - 0.005f, 0 - 0.005f, 0).color(255, 255, 255, 255).uv(1, 0).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(0).normal(normalMatrix, 0, 0, -1).endVertex();
    ...
    @Override
    public void onPreBlockEntityRender(PoseStack matrices, MultiBufferSource consumer, Level level, float partialTicks) {
        if (!Minecraft.getInstance().options.hideGui && !MCXRPlayClient.INSTANCE.MCXRGuiManager.isScreenOpen()) {
            ...
            if (hitResult != null && !MCXRPlayClient.INSTANCE.MCXRGuiManager.isScreenOpen()) {
                ...
                VertexConsumer vertexConsumer = consumer.getBuffer(cursorLayer);
            ...
            if (camera.getEntity() instanceof LocalPlayer player) {
                this.render(
                        player,
                        VrFirstPersonRenderer.getLight(camera, level),
                        matrices,
                        consumer,
                        partialTicks
                );
    ...
    @SubscribeEvent
    public void onLastRender(RenderLevelLastEvent event) {
